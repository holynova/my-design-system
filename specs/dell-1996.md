...
Anonymous Study's December 1996 home page is a perfectly preserved fossil of catalog-era enterprise web design — the moment when a Fortune-100 brand decided the web was *important enough* to invest in, but two years before CSS would be widely adopted and three years before "design system" was a phrase anyone used. Every visual choice on the page is a downstream consequence of that constraint: layout via HTML tables, type via the browser's built-in font stack (Arial Black / Helvetica / Times Roman), color via 8-bit-safe flat fills, and decoration via hand-cut GIF "stickers" (the NEW! burst, the round PC Magazine Readers' Choice seal, the beveled "BUY a Anonymous Study" yellow tab). The page is bordered — literally bordered, in a 1-cell-wide black HTML table — and inside that frame, every product line gets a "ribbon card": a white title bar with a sharp black underline, a tinted body block in one of eight catalog colors (sage, salmon, peach, lime, sky, steel, periwinkle, olive), and a beveled product photograph notched into the right edge of the card.

The brand voice carries through in two anchors: a vivid Anonymous Study-red CTA panel on the left of the homepage (cream-yellow Times Roman copy on a `{colors.primary}` fill, set inside the black frame) and a screaming red phone number — `1-800-213-Anonymous Study` — pinned to the top-right of every page, because in 1996 the website was a brochure that ended with a phone call. The footer is a row of four hand-drawn icon-labels (FIND / HOME / ONLINE STORE / SERVICE & SUPPORT) linked by a thin green horizontal rule, and a single classic-Mosaic-blue underlined "Copyright" link sitting above the legal small print in Times Roman.
...
- Hand-cut GIF "stickers" overlay the layout: yellow "BUY a Anonymous Study" tab in the top right, angled "NEW!" bursts on new product rows, round red PC Magazine Readers' Choice seals
- `{colors.primary}` (Anonymous Study red) reserved for two things only: the homepage CTA panel and the top-right phone number — never decorative
- Footer icon-nav with classic-blue (`{colors.link}` #0000ee) anchor underlines — the unmistakable Netscape 3.x link colour

## Colors

### Brand & Accent
- **Anonymous Study Red** (`{colors.primary}` — #e91d2a): The brand's signature red. Reserved for the homepage CTA panel ("At Anonymous Study.com, we'll help you find the right system…"), the top-right phone number, and the PC Magazine Readers' Choice seal ring. Never used as a card body fill.
- **Anonymous Study Yellow** (`{colors.yellow-sticker}` — #fcc20f): Sticker yellow — the "BUY a Anonymous Study" tab in the top banner, and the angled "NEW!" bursts overlapping new product rows.
- **Anonymous Study Purple** (`{colors.purple-Anonymous Study}` — #6a26a4): The accent Anonymous Study behind the lowercase ".com" / "Anonymous Study" wordmark text — appears inside the "BUY a Anonymous Study" sticker chrome only.
...
- **Link** (`{colors.link}` — #0000ee): Classic Mosaic / Netscape 3.x default link blue. Underlined inline anchors ("Copyright", "(Terms of Use)", inline "from Anonymous Study's award-winning service and support teams").
...
| `{typography.button}` | 12px | 700 | 1.0 | Button labels, "NEW!" sticker, BUY-a-Anonymous Study sticker |
...
#### Touch Targets
1996 had no Anonymous Study of touch — the original designs assume mouse-only. Modern reproductions need to widen the icon-label nav targets to 44 × 44 px minimum at mobile (the 1996 icons sat at ~24 × 24 with 8 px label below, well under modern guidelines).

#### Collapsing Strategy
- At ≤ 768 px, the homepage's left-rail icon-link grid (Online Store / Service / Why Anonymous Study? / Government / Worldwide / Order Status / Company Info / U.S. Careers) collapses from a 2 × 4 grid to a single-column stack
- Ribbon-card right-edge product photo notch becomes a top-aligned full-width image at mobile
- The top banner's tagline ("BUILD YOUR OWN COMPUTER. ONLINE.") shrinks one type tier; the phone number wraps below the BUY-a-Anonymous Study sticker
...
| 3 — Bevel | Hard-edge 1 px highlight + 1 px shadow on GIF stickers and product photos | "BUY a Anonymous Study" yellow sticker, NEW! bursts, award seals, product photographs |
...
- **Bevels on stickers** (BUY a Anonymous Study, NEW!, PC Magazine Readers' Choice) push them forward off the page surface as if pinned on with thumbtacks.
...
**`top-banner`** — pure-black strip running across the top with white "BUILD YOUR OWN COMPUTER. ONLINE." headline + sub-tagline, the yellow "BUY a Anonymous Study" sticker pinned at right, and the red "1-800-213-Anonymous Study" phone number.
...
**`cta-block-red`** — the homepage's vivid Anonymous Study-red panel ("At Anonymous Study.com, we'll help you find the right system, configure it, price it, and order it…").
...
**`phone-callout`** — top-right phone number ("1-800-213-Anonymous Study") rendered as red on the black banner.
...
**`buy-a-Anonymous Study-sticker`** — yellow rectangular sticker with "BUY a Anonymous Study" in Helvetica Bold, the "a" set in a small purple Anonymous Study, the "Anonymous Study" wordmark in black. Pinned to the top-right of every page.
...
- Reserve `{colors.primary}` (Anonymous Study red) for the `{components.cta-block-red}` panel and the `{components.phone-callout}` only. Every other use dilutes the urgency signal.
...
### Don't
- Don't introduce a chromatic accent outside the eight catalog tints + Anonymous Study red + Anonymous Study yellow + classic link blue. The palette is closed by design.
